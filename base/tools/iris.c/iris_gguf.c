...
static uint64_t cur_read(gguf_cursor_t *c, void *dst, size_t n) {
    if (c->error || (size_t)(c->end - c->p) < n) { c->error = 1; return 0; }
    if (dst) memcpy(dst, c->p, n);
    c->p += n;
    return 1;
}

static uint32_t cur_u32(gguf_cursor_t *c) { uint32_t v = 0; cur_read(c, &v, 4); return v; }
static uint64_t cur_u64(gguf_cursor_t *c) { uint64_t v = 0; cur_read(c, &v, 8); return v; }
...
static uint64_t cur_str(gguf_cursor_t *c, char *out, size_t max) {
    ...
    if (c->error || (size_t)(c->end - c->p) < len) { c->error = 1; return 0; }
    if (out) {
...
static void gguf_read_value(gguf_cursor_t *c, uint32_t type, int want_align,
                            uint32_t *align_out) {
    if (type == GGUF_T_STRING) {
        ...
        if (sz == 0) { c->error = 1; return; }
        uint64_t v = 0;
        cur_read(c, &v, sz);
        if (want_align && type == GGUF_T_UINT32 && align_out) *align_out = (uint32_t)v;
    }
}

static safetensor_dtype_t gguf_map_dtype(uint32_t ggml_type, size_t *elem_size) {
    switch (ggml_type) {
        case GGML_TYPE_F32:  *elem_size = 4; return DTYPE_F32;
        case GGML_TYPE_F16:  *elem_size = 2; return DTYPE_F16;
        case GGML_TYPE_BF16: *elem_size = 2; return DTYPE_BF16;
        case GGML_TYPE_Q8_0: *elem_size = 0; return DTYPE_Q8_0;  /* block type, see below */
        default:             *elem_size = 0; return DTYPE_UNKNOWN;
    }
}

safetensors_file_t *gguf_open(const char *path) {
    int fd = open(path, O_RDONLY);
    if (fd < 0) { perror("gguf_open: open failed"); return NULL; }

    struct stat st;
    if (fstat(fd, &st) < 0) { perror("gguf_open: fstat failed"); close(fd); return NULL; }
    size_t file_size = (size_t)st.st_size;
    if (file_size < 24) { fprintf(stderr, "gguf_open: file too small\n"); close(fd); return NULL; }

    void *data = mmap(NULL, file_size, PROT_READ, MAP_PRIVATE, fd, 0);
    close(fd);
    if (data == MAP_FAILED) { perror("gguf_open: mmap failed"); return NULL; }
    ...
    for (uint64_t i = 0; i < n_tensors && !c.error; i++) {
        ...
        if (ndim > 8) { c.error = 1; break; }

        /* GGML stores dims fastest-first; reverse to PyTorch (slowest-first). */
        int64_t ne[8];
        for (uint32_t d = 0; d < ndim; d++) ne[d] = (int64_t)cur_u64(&c);
        t->ndim = (int)ndim;
        for (uint32_t d = 0; d < ndim; d++) t->shape[d] = ne[ndim - 1 - d];
        ...
        if (t->dtype == DTYPE_UNKNOWN) {
            fprintf(stderr, "gguf_open: %s: unsupported ggml type %u "
        ...
        int64_t numel = 1;
        for (uint32_t d = 0; d < ndim; d++) numel *= ne[d];
    ...
    for (int i = 0; i < nt; i++) {
        ...
        if (data_start + t->data_offset + t->data_size > file_size) {
            fprintf(stderr, "gguf_open: %s: tensor '%s' extends past end of file "
                    "(truncated download?)\n", path, t->name);
            safetensors_close(sf);
            return NULL;
        }
