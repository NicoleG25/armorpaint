...
typedef struct { uint32_t n; } unary_pc_t;
typedef struct { uint32_t channels, H, W; } upsample_pc_t;

typedef struct { uint32_t rows, hidden; float eps; } rmsnorm_pc_t;
typedef struct { uint32_t rows, num_heads, head_dim; float eps; } headrms_pc_t;
typedef struct { uint32_t seq, num_q_heads, num_kv_heads, head_dim; } rope_pc_t;
typedef struct { uint32_t seq, num_heads, num_kv_heads, head_dim; float scale; } attn_pc_t;

/* Resident-op push constants. */
typedef struct { uint32_t seq, heads, head_dim; float eps; } res_qkrms_pc_t;
typedef struct { uint32_t seq, heads, head_dim; } res_rope_pc_t;
typedef struct { uint32_t seq, num_heads, head_dim; float scale; } res_attn_pc_t;
typedef struct { uint32_t seq, width, fused_dim, n_streams; } res_split_pc_t;
typedef struct { uint32_t seq, hidden; } res_gatedadd_pc_t;
typedef struct { uint32_t seq, in_dim, out_dim, has_bias; } res_linf32_pc_t;
typedef struct { uint32_t seq, w, src_stride, src_off, dst_stride, dst_off; } flux_rowcopy_pc_t;
...
static int find_memory_type(uint32_t type_bits, VkMemoryPropertyFlags want,
                            uint32_t *out_index) {
...
static int buffer_ensure(vk_buffer_t *b, VkDeviceSize bytes,
                         VkBufferUsageFlags usage, int host_visible) {
    if (bytes == 0) bytes = 16;
    if (b->buffer != VK_NULL_HANDLE && b->capacity >= bytes) return 1;

    /* Grow geometrically to avoid frequent reallocation as sizes ratchet up. */
    if (b->buffer != VK_NULL_HANDLE && b->capacity * 2 > bytes) bytes = b->capacity * 2;

    if (b->buffer != VK_NULL_HANDLE) {
        if (b->map) { vkUnmapMemory(g.device, b->mem); b->map = NULL; }
    ...
    if (!find_memory_type(req.memoryTypeBits, want, &mtype)) {
        ...
        if (host_visible ||
            !find_memory_type(req.memoryTypeBits,
                              ...
                              VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &mtype)) {
...
static VkBufferUsageFlags compute_usage(void) {
    ...
    if (g.discrete) u |= VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
    return u;
}
...
static int device_upload(vk_buffer_t *dev, const void *data, size_t bytes) {
    if (!buffer_ensure(&g.stage_w, bytes, STAGE_W_USAGE, 1)) return 0;
...
static int weight_upload(vk_buffer_t *buf, const void *data, size_t copy_bytes) {
    ...
    if (g.discrete) {
        if (!buffer_ensure(buf, alloc_bytes, compute_usage(), 0)) return 0;
        return device_upload(buf, data, copy_bytes);
    }
    if (!buffer_ensure(buf, alloc_bytes, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, 1)) return 0;
...
static vk_buffer_t *weight_get(const void *key, const void *data, size_t copy_bytes) {
    ...
    for (int i = 0; i < g.weights_count; i++) {
        if (g.weights[i].key != key) continue;
        weight_entry_t *e = &g.weights[i];
        if (e->fingerprint == fp && e->bytes == copy_bytes) return &e->buf;  /* valid hit */
        /* Stale: address reused for a different weight -> re-upload. */
        if (!weight_upload(&e->buf, data, copy_bytes)) return NULL;
        e->fingerprint = fp;
        e->bytes = copy_bytes;
        return &e->buf;
    }
...
void iris_vulkan_release_weight_cache(void) {
    if (!g.initialized || g.device == VK_NULL_HANDLE) return;
...
static int create_pipeline(const uint32_t *spv, size_t spv_bytes,
...
static void init_vae_resident(void) {
    ...
    dli.bindingCount = 5;
    dli.pBindings = binds;
    if (vkCreateDescriptorSetLayout(g.device, &dli, NULL, &g.vae_dset_layout) != VK_SUCCESS) return;
    ...
    if (vkCreatePipelineLayout(g.device, &pli, NULL, &g.vae_pipe_layout) != VK_SUCCESS) return;
    ...
    dpi.poolSizeCount = 1;
    dpi.pPoolSizes = &ps;
    if (vkCreateDescriptorPool(g.device, &dpi, NULL, &g.vae_dpool) != VK_SUCCESS) return;
    ...
    if (vkAllocateCommandBuffers(g.device, &cbi, &g.vae_cmd) != VK_SUCCESS) return;

    if (!create_pipeline(conv2d_spv, sizeof(conv2d_spv), g.vae_pipe_layout, &g.pipe_conv2d)) return;
    if (!create_pipeline(groupnorm_spv, sizeof(groupnorm_spv), g.vae_pipe_layout, &g.pipe_groupnorm)) return;
    if (!create_pipeline(swish_spv, sizeof(swish_spv), g.vae_pipe_layout, &g.pipe_swish)) return;
    if (!create_pipeline(add_spv, sizeof(add_spv), g.vae_pipe_layout, &g.pipe_add)) return;
    if (!create_pipeline(upsample_spv, sizeof(upsample_spv), g.vae_pipe_layout, &g.pipe_upsample)) return;
    ...
    g.flux_available = 0;
    if (g.res_available &&
        create_pipeline(flux_rowcopy_spv, sizeof(flux_rowcopy_spv), g.vae_pipe_layout, &g.pipe_flux_rowcopy)) {
        g.flux_available = 1;
    }
}
...
int iris_vulkan_init(void) {
    if (g.initialized) return g.available;
    g.initialized = 1;
    g.available = 0;

    /* Escape hatch: force the CPU path (for debugging / unsupported drivers). */
    if (getenv("IRIS_VK_DISABLE")) {
        if (iris_verbose) fprintf(stderr, "Vulkan: disabled via IRIS_VK_DISABLE\n");
        return 0;
    }
    ...
    if (vkCreateInstance(&ici, NULL, &g.instance) != VK_SUCCESS) {
        if (iris_verbose) fprintf(stderr, "Vulkan: no instance (loader/driver missing)\n");
        return 0;
    }
    ...
    if (n == 0) {
        if (iris_verbose) fprintf(stderr, "Vulkan: no physical devices\n");
        return 0;
    }
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        for (uint32_t q = 0; q < qn; q++) {
            if (qf[q].queueFlags & VK_QUEUE_COMPUTE_BIT) { cf = (int)q; break; }
        }
        free(qf);
        if (cf < 0) continue;
    ...
    if (g.phys == VK_NULL_HANDLE) {
        if (iris_verbose) fprintf(stderr, "Vulkan: no compute-capable device\n");
        return 0;
    }
    ...
    if (getenv("IRIS_VK_NOSTAGE")) g.discrete = 0;
    if (iris_verbose)
    ...
    if (!create_pipeline(gemm_f32_spv, sizeof(gemm_f32_spv), g.pipe_layout, &g.pipe_f32)) return 0;
    if (!create_pipeline(gemm_bf16_spv, sizeof(gemm_bf16_spv), g.pipe_layout, &g.pipe_bf16)) return 0;
    if (!create_pipeline(gemm_q8_spv, sizeof(gemm_q8_spv), g.pipe_layout, &g.pipe_q8)) return 0;
...
static void vk_gemm(int wtype, int b_cached,
    ...
    size_t a_elems = transpose_a ? (size_t)(K - 1) * lda + M
                                 : (size_t)(M - 1) * lda + K;
    size_t b_elems = transpose_b ? (size_t)(N - 1) * ldb + K
                                 : (size_t)(K - 1) * ldb + N;
    ...
    size_t b_copy = (wtype == 2) ? (b_elems / 32) * 34
                  : (wtype == 1) ? (b_elems * 2)
                                 : (b_elems * 4);
    ...
    if (!compute_buffer_ensure(&g.buf_a, a_bytes)) return;
    if (!compute_buffer_ensure(&g.buf_c, c_bytes)) return;
    if (staged) {
        if (!buffer_ensure(&g.stage_a, a_bytes, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, 1)) return;
        if (!buffer_ensure(&g.stage_c, c_bytes,
                           VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, 1)) return;
    }
    ...
    if (beta != 0.0f) {
        if (ldc == N) {
            ...
            for (int r = 0; r < M; r++)
                memcpy((float *)c_host + (size_t)r * ldc,
                       C + (size_t)r * ldc, (size_t)N * 4);
        }
    }
    ...
    if (b_cached) {
        ...
        if (!compute_buffer_ensure(&g.buf_b, b_alloc)) return;
        if (staged) {
            if (!buffer_ensure(&g.stage_b, b_alloc, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, 1)) return;
    ...
    gemm_pc_t pc;
    pc.M = (uint32_t)M; pc.N = (uint32_t)N; pc.K = (uint32_t)K;
    pc.lda = (uint32_t)lda; pc.ldb = (uint32_t)ldb; pc.ldc = (uint32_t)ldc;
    pc.ta = transpose_a ? 1u : 0u; pc.tb = transpose_b ? 1u : 0u;
    ...
    vkCmdBindPipeline(g.cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                      (wtype == 2) ? g.pipe_q8 :
                      (wtype == 1) ? g.pipe_bf16 : g.pipe_f32);
    vkCmdBindDescriptorSets(g.cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                            g.pipe_layout, 0, 1, &g.dset, 0, NULL);
    vkCmdPushConstants(g.cmd, g.pipe_layout, VK_SHADER_STAGE_COMPUTE_BIT,
                       0, sizeof(pc), &pc);
...
void iris_vulkan_sgemm(int transpose_a, int transpose_b,
                       ...
                       float beta,
                       float *C, int ldc) {
    vk_gemm(0, 0, transpose_a, transpose_b, M, N, K,
...
void iris_vulkan_sgemm_cached(int transpose_a, int transpose_b,
                              ...
                              float beta,
                              float *C, int ldc) {
    vk_gemm(0, 1, transpose_a, transpose_b, M, N, K,
            alpha, A, lda, B, ldb, beta, C, ldc);
}
...
void iris_vulkan_sgemm_bf16(int transpose_a, int transpose_b,
                            ...
                            float beta,
                            float *C, int ldc) {
    vk_gemm(1, 1, transpose_a, transpose_b, M, N, K,
            alpha, A, lda, B_bf16, ldb, beta, C, ldc);
}
...
void iris_vulkan_sgemm_q8(int transpose_a, int transpose_b,
                          ...
                          float beta,
                          float *C, int ldc) {
    vk_gemm(2, 1, transpose_a, transpose_b, M, N, K,
            alpha, A, lda, B_q8, ldb, beta, C, ldc);
}
...
static VkCommandBuffer vae_cmd_begin(void) {
    if (g.vae_recording) return g.vae_cmd;
...
static void resident_dispatch(VkPipeline pipe, const VkBuffer b[5],
                              const void *pc, uint32_t pc_size,
                              uint32_t gx, uint32_t gy) {
    VkCommandBuffer cmd = vae_cmd_begin();
    VkDescriptorSet set = vae_alloc_set();
    if (set == VK_NULL_HANDLE) { vae_cmd_finish(); return; }
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                            g.vae_pipe_layout, 0, 1, &set, 0, NULL);
...
static void vae_dispatch(VkPipeline pipe, VkBuffer b0, VkBuffer b1, VkBuffer b2, VkBuffer b3,
                         const void *pc, uint32_t pc_size, uint32_t group_count) {
...
iris_gpu_tensor_t iris_gpu_tensor_alloc(size_t num_elements) {
    ...
    if (!t) return NULL;
    t->n = num_elements;
    if (!tensor_buf_alloc(&t->buf, num_elements * 4)) { free(t); return NULL; }
    return t;
}
...
void iris_gpu_tensor_free(iris_gpu_tensor_t t) {
    ...
    if (g.vae_recording) {
        if (g.vae_pending_count == g.vae_pending_cap) {
            ...
            if (np) { g.vae_pending_free = np; g.vae_pending_cap = nc; }
        }
        if (g.vae_pending_count < g.vae_pending_cap) {
...
void iris_gpu_tensor_read(iris_gpu_tensor_t t, float *out) {
    if (!t) return;
    size_t bytes = t->n * 4;
    if (g.discrete) device_download(&t->buf, out, bytes);
    else memcpy(out, t->buf.map, bytes);
}

void iris_gpu_tensor_write(iris_gpu_tensor_t t, const float *data) {
    if (!t) return;
    size_t bytes = t->n * 4;
    if (g.discrete) device_upload(&t->buf, data, bytes);
    else memcpy(t->buf.map, data, bytes);
}

void iris_gpu_copy_f32(iris_gpu_tensor_t dst, iris_gpu_tensor_t src, size_t n) {
    if (!g.vae_available || !dst || !src) return;
    VkCommandBuffer cmd = vae_cmd_begin();
...
iris_gpu_tensor_t iris_gpu_conv2d_f32(iris_gpu_tensor_t x,
    ...
    if (!g.vae_available || !x) return NULL;
    ...
    if (!wb) { iris_gpu_tensor_free(out); return NULL; }
    VkBuffer bias_buf;
    if (bias) {
        vk_buffer_t *bb = weight_get(bias, bias, (size_t)out_ch * 4);
        if (!bb) { iris_gpu_tensor_free(out); return NULL; }
        bias_buf = bb->buffer;
    } else {
        bias_buf = x->buf.buffer;  /* unused by shader when has_bias == 0 */
    }

    conv_pc_t pc = {(uint32_t)batch, (uint32_t)in_ch, (uint32_t)out_ch,
                    (uint32_t)H, (uint32_t)W, (uint32_t)kH, (uint32_t)kW,
                    (uint32_t)stride, (uint32_t)padding,
                    (uint32_t)outH, (uint32_t)outW, bias ? 1u : 0u,
                    iris_circular ? 1u : 0u};
    vae_dispatch(g.pipe_conv2d, x->buf.buffer, wb->buffer, bias_buf, out->buf.buffer,
                 &pc, sizeof(pc), vae_groups(out_n));
    return out;
}
...
void iris_gpu_group_norm_f32(iris_gpu_tensor_t out, iris_gpu_tensor_t x,
    ...
    if (!g.vae_available || !out || !x) return;
    vk_buffer_t *gw = weight_get(gamma, gamma, (size_t)channels * 4);
    vk_buffer_t *bw = weight_get(beta, beta, (size_t)channels * 4);
    if (!gw || !bw) return;
    gnorm_pc_t pc = {(uint32_t)batch, (uint32_t)channels, (uint32_t)spatial,
                     (uint32_t)num_groups, eps};
    /* One workgroup per (batch, group). */
    vae_dispatch(g.pipe_groupnorm, x->buf.buffer, gw->buffer, bw->buffer, out->buf.buffer,
                 &pc, sizeof(pc), (uint32_t)(batch * num_groups));
}

void iris_gpu_swish_f32(iris_gpu_tensor_t out, iris_gpu_tensor_t x, int n) {
    if (!g.vae_available || !out || !x) return;
    unary_pc_t pc = {(uint32_t)n};
    vae_dispatch(g.pipe_swish, x->buf.buffer, out->buf.buffer, x->buf.buffer, x->buf.buffer,
                 &pc, sizeof(pc), vae_groups((size_t)n));
}

void iris_gpu_add_f32(iris_gpu_tensor_t out, iris_gpu_tensor_t a, iris_gpu_tensor_t b, int n) {
    if (!g.vae_available || !out || !a || !b) return;
    unary_pc_t pc = {(uint32_t)n};
    vae_dispatch(g.pipe_add, a->buf.buffer, b->buf.buffer, out->buf.buffer, a->buf.buffer,
                 &pc, sizeof(pc), vae_groups((size_t)n));
}
...
iris_gpu_tensor_t iris_gpu_upsample_nearest_2x_f32(iris_gpu_tensor_t x,
    ...
    vae_dispatch(g.pipe_upsample, x->buf.buffer, out->buf.buffer, x->buf.buffer, x->buf.buffer,
                 &pc, sizeof(pc), vae_groups(out_n));
    return out;
}
...
void iris_vk_qwen_linear(iris_gpu_tensor_t out, iris_gpu_tensor_t x,
    ...
    if (!g.qwen_available || !out || !x || !weight_bf16) return;
    vk_buffer_t *wb = weight_get(weight_bf16, weight_bf16,
                                 (size_t)out_dim * in_dim * 2);
    if (!wb) return;
    gemm_pc_t pc;
    pc.M = (uint32_t)seq; pc.N = (uint32_t)out_dim; pc.K = (uint32_t)in_dim;
    pc.lda = (uint32_t)in_dim; pc.ldb = (uint32_t)in_dim; pc.ldc = (uint32_t)out_dim;
    pc.ta = 0u; pc.tb = 1u;          /* C = x @ W^T (W is [out, in]) */
    pc.alpha = 1.0f; pc.beta = 0.0f;
    VkBuffer b[5] = {x->buf.buffer, wb->buffer, out->buf.buffer,
                     out->buf.buffer, out->buf.buffer};
    resident_dispatch(g.pipe_qwen_linear, b, &pc, sizeof(pc),
                      ((uint32_t)out_dim + 15u) / 16u, ((uint32_t)seq + 15u) / 16u);
}

void iris_vk_qwen_linear_q8(iris_gpu_tensor_t out, iris_gpu_tensor_t x,
                            const void *weight_q8,
                            int seq, int in_dim, int out_dim) {
    if (!g.qwen_available || !out || !x || !weight_q8) return;
    ...
    VkBuffer b[5] = {x->buf.buffer, wb->buffer, out->buf.buffer,
                     out->buf.buffer, out->buf.buffer};
    resident_dispatch(g.pipe_qwen_linear_q8, b, &pc, sizeof(pc),
                      ((uint32_t)out_dim + 15u) / 16u, ((uint32_t)seq + 15u) / 16u);
}
...
void iris_vk_qwen_head_rms_norm(iris_gpu_tensor_t t,
    ...
    if (!g.qwen_available || !t || !weight_bf16) return;
    ...
    VkBuffer b[5] = {t->buf.buffer, wb->buffer, t->buf.buffer,
                     t->buf.buffer, t->buf.buffer};
    resident_dispatch(g.pipe_qwen_headrms, b, &pc, sizeof(pc),
                      vae_groups((size_t)seq * num_heads), 1);
}
...
void iris_vk_qwen_rope(iris_gpu_tensor_t q, iris_gpu_tensor_t k,
    ...
    if (!g.qwen_available || !q || !k || !cos_table || !sin_table) return;
    int half = head_dim / 2;
    ...
    if (!cb || !sb) return;
    rope_pc_t pc = {(uint32_t)seq, (uint32_t)num_q_heads,
                    (uint32_t)num_kv_heads, (uint32_t)head_dim};
    VkBuffer b[5] = {q->buf.buffer, k->buf.buffer, cb->buffer, sb->buffer,
                     q->buf.buffer};
...
void iris_vk_qwen_attention(iris_gpu_tensor_t out,
    ...
    if (!g.qwen_available || !out || !q || !k || !v || !mask) return;
    attn_pc_t pc = {(uint32_t)seq, (uint32_t)num_heads,
                    (uint32_t)num_kv_heads, (uint32_t)head_dim, scale};
    VkBuffer b[5] = {q->buf.buffer, k->buf.buffer, v->buf.buffer,
                     out->buf.buffer, mask->buf.buffer};
    /* One workgroup per (query position, head). */
    resident_dispatch(g.pipe_qwen_attn, b, &pc, sizeof(pc),
                      (uint32_t)seq, (uint32_t)num_heads);
}

void iris_vk_qwen_silu_mul(iris_gpu_tensor_t gate, iris_gpu_tensor_t up, int n) {
    if (!g.qwen_available || !gate || !up) return;
    unary_pc_t pc = {(uint32_t)n};
    VkBuffer b[5] = {gate->buf.buffer, up->buf.buffer, gate->buf.buffer,
                     gate->buf.buffer, gate->buf.buffer};
...
iris_gpu_tensor_t iris_gpu_tensor_alloc_f16(size_t num_elements) {
    ...
    t->n = num_elements;
    t->is_f16 = 1;
    if (!tensor_buf_alloc(&t->buf, num_elements * 2)) { free(t); return NULL; }
    return t;
}

float *iris_gpu_tensor_data(iris_gpu_tensor_t t) {
    if (!t) return NULL;
    if (!g.discrete) return (float *)t->buf.map;
...
void iris_gpu_tensor_set_persistent(iris_gpu_tensor_t t, int persistent) {
    (void)t; (void)persistent;  /* no pool under Vulkan; tensors freed explicitly */
}
...
void iris_gpu_rms_norm_f32(iris_gpu_tensor_t out, iris_gpu_tensor_t x,
    ...
    if (!g.res_available || !out || !x || !weight) return;
    VkCommandBuffer cmd = vae_cmd_begin();
    VkBuffer wbuf = dyn_upload(cmd, weight, (size_t)hidden * 4);
    if (wbuf == VK_NULL_HANDLE) { vae_cmd_finish(); return; }
...
void iris_gpu_qk_rms_norm(iris_gpu_tensor_t q, iris_gpu_tensor_t k,
    ...
    if (!g.res_available || !q || !k || !q_weight || !k_weight) return;
    ...
    resident_dispatch(g.pipe_res_qkrmsnorm, b, &pc, sizeof(pc),
                      vae_groups((size_t)seq * heads), 1);
}
...
void iris_gpu_rope_single_pair_f32(iris_gpu_tensor_t q, iris_gpu_tensor_t k,
    ...
    if (!g.res_available || !q || !k || !cos_freq || !sin_freq) return;
...
void iris_gpu_split_qkv_mlp(iris_gpu_tensor_t fused,
    ...
    if (!g.res_available || !fused) return;
    if (hidden > 0 && q && k && v) {
        res_split_pc_t pc = {(uint32_t)seq, (uint32_t)hidden, (uint32_t)(3 * hidden), 3u};
        VkBuffer b[5] = {fused->buf.buffer, q->buf.buffer, k->buf.buffer,
                         v->buf.buffer, q->buf.buffer};
        resident_dispatch(g.pipe_res_split, b, &pc, sizeof(pc),
                          ((uint32_t)hidden + 15u) / 16u, ((uint32_t)seq + 15u) / 16u);
    }
    if (mlp_hidden > 0 && gate && up) {
        res_split_pc_t pc = {(uint32_t)seq, (uint32_t)mlp_hidden, (uint32_t)(2 * mlp_hidden), 2u};
        VkBuffer b[5] = {fused->buf.buffer, gate->buf.buffer, up->buf.buffer,
                         gate->buf.buffer, gate->buf.buffer};
        resident_dispatch(g.pipe_res_split, b, &pc, sizeof(pc),
                          ((uint32_t)mlp_hidden + 15u) / 16u, ((uint32_t)seq + 15u) / 16u);
    }
}
...
void iris_gpu_adaln_norm(iris_gpu_tensor_t out, iris_gpu_tensor_t x,
                         ...
                         int seq, int hidden, float eps) {
    if (!g.res_available || !out || !x || !shift || !scale) return;
    VkCommandBuffer cmd = vae_cmd_begin();
    VkBuffer sh = dyn_upload(cmd, shift, (size_t)hidden * 4);
    VkBuffer sc = dyn_upload(cmd, scale, (size_t)hidden * 4);
    if (sh == VK_NULL_HANDLE || sc == VK_NULL_HANDLE) { vae_cmd_finish(); return; }
...
void iris_gpu_gated_add(iris_gpu_tensor_t out, const float *gate,
    ...
    if (!g.res_available || !out || !gate || !proj) return;
    VkCommandBuffer cmd = vae_cmd_begin();
    VkBuffer gbuf = dyn_upload(cmd, gate, (size_t)hidden * 4);
    if (gbuf == VK_NULL_HANDLE) { vae_cmd_finish(); return; }
    ...
    resident_dispatch(g.pipe_res_gatedadd, b, &pc, sizeof(pc),
                      ((uint32_t)hidden + 15u) / 16u, ((uint32_t)seq + 15u) / 16u);
}
...
int iris_gpu_linear_bf16_into(iris_gpu_tensor_t out, iris_gpu_tensor_t x,
    ...
    if (!g.res_available || !out || !x || !W_bf16) return 0;
...
int iris_gpu_linear_q8_into(iris_gpu_tensor_t out, iris_gpu_tensor_t x,
                            const void *W_q8,
                            int seq_len, int in_dim, int out_dim) {
    if (!g.res_available || !out || !x || !W_q8) return 0;
    iris_vk_qwen_linear_q8(out, x, W_q8, seq_len, in_dim, out_dim);
    return 1;
}

iris_gpu_tensor_t iris_gpu_linear_bf16(iris_gpu_tensor_t x, const uint16_t *W_bf16,
                                       int seq_len, int in_dim, int out_dim) {
    if (!g.res_available || !x || !W_bf16) return NULL;
...
iris_gpu_tensor_t iris_gpu_linear(iris_gpu_tensor_t x, const float *W, const float *b_bias,
                                  int seq_len, int in_dim, int out_dim) {
    if (!g.res_available || !x || !W) return NULL;
    iris_gpu_tensor_t out = iris_gpu_tensor_alloc((size_t)seq_len * out_dim);
    if (!out) return NULL;
    ...
    res_linf32_pc_t pc = {(uint32_t)seq_len, (uint32_t)in_dim, (uint32_t)out_dim,
                         b_bias ? 1u : 0u};
    VkBuffer b[5] = {x->buf.buffer, wb->buffer, bias_buf, out->buf.buffer, out->buf.buffer};
    resident_dispatch(g.pipe_res_linear_f32, b, &pc, sizeof(pc),
                      ((uint32_t)out_dim + 15u) / 16u, ((uint32_t)seq_len + 15u) / 16u);
    return out;
}
...
int iris_gpu_attention_fused(iris_gpu_tensor_t out,
    ...
    if (!g.res_available || !out || !Q || !K || !V) return 0;
    if (seq_q != seq_k) return 0;  /* full square self-attention only */
    ...
    resident_dispatch(g.pipe_res_attn, b, &pc, sizeof(pc),
                      (uint32_t)seq_q, (uint32_t)num_heads);
    return 1;
}
...
int iris_gpu_attention_bf16(iris_gpu_tensor_t out, iris_gpu_tensor_t Q,
    ...
    (void)num_heads; (void)head_dim; (void)scale;
    return 0;
}
...
void iris_gpu_row_copy_f32(iris_gpu_tensor_t dst, int dst_stride, int dst_off,
                           ...
                           int seq, int w) {
    if (!g.flux_available || !dst || !src) return;
    flux_rowcopy_pc_t pc = {(uint32_t)seq, (uint32_t)w,
                            (uint32_t)src_stride, (uint32_t)src_off,
                            (uint32_t)dst_stride, (uint32_t)dst_off};
    VkBuffer b[5] = {src->buf.buffer, dst->buf.buffer, dst->buf.buffer,
                     dst->buf.buffer, dst->buf.buffer};
    resident_dispatch(g.pipe_flux_rowcopy, b, &pc, sizeof(pc),
                      ((uint32_t)w + 15u) / 16u, ((uint32_t)seq + 15u) / 16u);
}

void iris_gpu_batch_begin(void) {
    if (!g.vae_available || g.vae_recording) return;
    vkResetDescriptorPool(g.device, g.vae_dpool, 0);
...
void iris_vulkan_cleanup(void) {
    ...
    for (int i = 0; i < g.weights_count; i++) buffer_destroy(&g.weights[i].buf);
    free(g.weights);
    g.weights = NULL;
    g.weights_count = g.weights_cap = 0;

    for (int i = 0; i < g.dyn_count; i++) buffer_destroy(&g.dyn_bufs[i]);
    free(g.dyn_bufs);
    g.dyn_bufs = NULL;
    ...
    if (g.pipe_f32) vkDestroyPipeline(g.device, g.pipe_f32, NULL);
    if (g.pipe_bf16) vkDestroyPipeline(g.device, g.pipe_bf16, NULL);
    if (g.pipe_conv2d) vkDestroyPipeline(g.device, g.pipe_conv2d, NULL);
    if (g.pipe_groupnorm) vkDestroyPipeline(g.device, g.pipe_groupnorm, NULL);
    if (g.pipe_swish) vkDestroyPipeline(g.device, g.pipe_swish, NULL);
    if (g.pipe_add) vkDestroyPipeline(g.device, g.pipe_add, NULL);
    if (g.pipe_upsample) vkDestroyPipeline(g.device, g.pipe_upsample, NULL);
    if (g.pipe_qwen_linear) vkDestroyPipeline(g.device, g.pipe_qwen_linear, NULL);
    if (g.pipe_qwen_rmsnorm) vkDestroyPipeline(g.device, g.pipe_qwen_rmsnorm, NULL);
    if (g.pipe_qwen_headrms) vkDestroyPipeline(g.device, g.pipe_qwen_headrms, NULL);
    if (g.pipe_qwen_rope) vkDestroyPipeline(g.device, g.pipe_qwen_rope, NULL);
    if (g.pipe_qwen_attn) vkDestroyPipeline(g.device, g.pipe_qwen_attn, NULL);
    if (g.pipe_qwen_silumul) vkDestroyPipeline(g.device, g.pipe_qwen_silumul, NULL);
    if (g.pipe_res_rmsnorm) vkDestroyPipeline(g.device, g.pipe_res_rmsnorm, NULL);
    if (g.pipe_res_qkrmsnorm) vkDestroyPipeline(g.device, g.pipe_res_qkrmsnorm, NULL);
    if (g.pipe_res_rope) vkDestroyPipeline(g.device, g.pipe_res_rope, NULL);
    if (g.pipe_res_attn) vkDestroyPipeline(g.device, g.pipe_res_attn, NULL);
    if (g.pipe_res_split) vkDestroyPipeline(g.device, g.pipe_res_split, NULL);
    if (g.pipe_res_gatedadd) vkDestroyPipeline(g.device, g.pipe_res_gatedadd, NULL);
    if (g.pipe_res_adaln) vkDestroyPipeline(g.device, g.pipe_res_adaln, NULL);
    if (g.pipe_res_linear_f32) vkDestroyPipeline(g.device, g.pipe_res_linear_f32, NULL);
    if (g.pipe_flux_rowcopy) vkDestroyPipeline(g.device, g.pipe_flux_rowcopy, NULL);
    if (g.vae_dpool) vkDestroyDescriptorPool(g.device, g.vae_dpool, NULL);
    if (g.vae_pipe_layout) vkDestroyPipelineLayout(g.device, g.vae_pipe_layout, NULL);
    if (g.vae_dset_layout) vkDestroyDescriptorSetLayout(g.device, g.vae_dset_layout, NULL);
    if (g.dpool) vkDestroyDescriptorPool(g.device, g.dpool, NULL);
    if (g.pipe_layout) vkDestroyPipelineLayout(g.device, g.pipe_layout, NULL);
    if (g.dset_layout) vkDestroyDescriptorSetLayout(g.device, g.dset_layout, NULL);
    if (g.fence) vkDestroyFence(g.device, g.fence, NULL);
    if (g.xfer_fence) vkDestroyFence(g.device, g.xfer_fence, NULL);
    if (g.cmd_pool) vkDestroyCommandPool(g.device, g.cmd_pool, NULL);
    if (g.xfer_pool) vkDestroyCommandPool(g.device, g.xfer_pool, NULL);
