...
/*
 * Mark a weight pointer as a GGML Q8_0 block stream (34 bytes per 32 elements:
 * an fp16 scale followed by 32 int8 quants). Weights passed through the bf16
 * linear/sgemm APIs whose pointer was registered here are dequantized to
 * bf16/f16 on first GPU upload and cached, so a Q8_0 checkpoint runs through
 * the same path as a bf16 one. Called by the model loader for each Q8_0 weight.
 */
...
/* Single-stream RoPE for f32 tensors (consecutive-pair rotation).
