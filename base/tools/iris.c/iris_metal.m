...
static int is_q8_weight(const void *ptr) {
    ...
    int found = 0;
    for (int i = 0; i < g_q8_registry_count; i++) {
        if (g_q8_registry[i] == ptr) { found = 1; break; }
    }
    pthread_mutex_unlock(&g_q8_registry_mutex);
    return found;
}
...
static void dequant_q8_0_block_stream(const void *q8, size_t num_elements,
                                      uint16_t *out, int out_is_f16) {
...
static id<MTLBuffer> get_cached_bf16_buffer(const uint16_t *weights, size_t num_elements) {
    ...
    if (g_bf16_cache_count >= BF16_WEIGHT_CACHE_SIZE) {
        id<MTLBuffer> buf = [g_device newBufferWithBytes:bytes_src
                                                  length:size
                                                 options:MTLResourceStorageModeShared];
        free(bf16_tmp);
        pthread_mutex_unlock(&g_bf16_cache_mutex);
...
static conv2d_graph_cache_t *get_conv2d_graph_cache(int batch, int in_ch, int out_ch,
                                                    ...
                                                    int stride, int padding, int circular) {
    ...
    @autoreleasepool {
        ...
        if (circular) {
            ...
            convSource = [graph padTensor:input
                          withPaddingMode:MPSGraphPaddingModePeriodic
                              leftPadding:lowPad
                             rightPadding:highPad
                            constantValue:0.0
                                     name:nil];
        ...
        MPSGraphTensor *conv = [graph convolution2DWithSourceTensor:convSource
