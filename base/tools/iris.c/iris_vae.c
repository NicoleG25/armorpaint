...
static iris_image *vae_decode_gpu(iris_vae_t *vae, const float *latent,
    ...
    if (!IRIS_GPU_RESIDENT_AVAILABLE()) return NULL;
    /* Escape hatch: force the CPU decoder (isolates resident-decode issues). */
    if (getenv("IRIS_VK_CPUVAE")) return NULL;
...
static iris_image *vae_decode_full(iris_vae_t *vae, const float *latent,
                                   int batch, int latent_h, int latent_w) {
#if defined(USE_METAL) || defined(USE_VULKAN)
...
static void extract_tile_latent(float *dst, const float *latent, int lat_ch,
                                int latent_h, int latent_w,
                                int li, int lj, int th, int tw, int wrap) {
    int z_spatial = latent_h * latent_w;
    for (int c = 0; c < lat_ch; c++) {
        ...
        for (int y = 0; y < th; y++) {
            int sy = li + y;
            if (wrap && sy >= latent_h) sy -= latent_h;
...
static iris_image *vae_decode_tiled(iris_vae_t *vae, const float *latent,
                                    int latent_h, int latent_w) {
    int lat_ch = vae->latent_channels;
    ...
    if (!accum || !wsum || !tile_latent) {
        free(accum); free(wsum); free(tile_latent);
        return NULL;
    }

    if (iris_verbose) {
        fprintf(stderr, "VAE tiling: %dx%d latent -> %dx%d image, %d-px tiles (%d overlap)%s\n",
                latent_w, latent_h, full_w, full_h, VAE_TILE_LATENT, VAE_TILE_OVERLAP,
                circular ? ", toroidal (seamless)" : "");
    }
    ...
    for (int li = 0; li < latent_h; li += stride) {
        ...
        if (!circular && li + th > latent_h) th = latent_h - li;

        for (int lj = 0; lj < latent_w; lj += stride) {
            int tw = VAE_TILE_LATENT;
            if (!circular && lj + tw > latent_w) tw = latent_w - lj;

            extract_tile_latent(tile_latent, latent, lat_ch, latent_h, latent_w,
                                li, lj, th, tw, circular);
            ...
            for (int y = 0; y < tH; y++) {
                float wy = 1.0f;
                if (ramp_top)    { float r = (y + 0.5f) / blend_px;       if (r < wy) wy = r; }
                if (ramp_bottom) { float r = (tH - y - 0.5f) / blend_px;  if (r < wy) wy = r; }
                if (wy < 0.0f) wy = 0.0f;

                int oy = oy0 + y;
                if (circular && oy >= full_h) oy -= full_h;

                for (int x = 0; x < tW; x++) {
                    float wx = 1.0f;
                    if (ramp_left)  { float r = (x + 0.5f) / blend_px;      if (r < wx) wx = r; }
                    if (ramp_right) { float r = (tW - x - 0.5f) / blend_px; if (r < wx) wx = r; }
                    if (wx < 0.0f) wx = 0.0f;

                    float w = wy * wx;
                    if (w <= 0.0f) w = 1e-6f;  /* keep coverage strictly positive */

                    int ox = ox0 + x;
                    if (circular && ox >= full_w) ox -= full_w;
    ...
    if (!img) { free(accum); free(wsum); free(tile_latent); return NULL; }
