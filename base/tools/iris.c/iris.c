...
iris_ctx *iris_load_dir(const char *model_dir) {
     ...
     * the model root, a sharded safetensors index, or a single safetensors file. */
    {
        ...
        snprintf(idx, sizeof(idx),
                 "%s/transformer/diffusion_pytorch_model.safetensors.index.json", model_dir);
        snprintf(st, sizeof(st),
                 "%s/transformer/diffusion_pytorch_model.safetensors", model_dir);
        if (!file_exists(gguf_q8) && !file_exists(gguf_bf16) &&
            !file_exists(idx) && !file_exists(st)) {
