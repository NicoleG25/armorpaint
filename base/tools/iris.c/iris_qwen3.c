     ...
     * Use BLAS for Q@K^T and scores@V matrix multiplications */
    {
        ...
        for (int h = 0; h < num_heads; h++) {
            ...
            if (iris_vulkan_available() &&
                (size_t)seq_len * seq_len >= QWEN3_VK_MIN_ELEMENTS) {
                iris_vulkan_sgemm(0, 1, seq_len, seq_len, head_dim,
            ...
            if (iris_vulkan_available() &&
                (size_t)seq_len * seq_len >= QWEN3_VK_MIN_ELEMENTS) {
                iris_vulkan_sgemm(0, 0, seq_len, head_dim, seq_len,
...
static void qwen3_vk_linear(qwen3_model_t *model, iris_gpu_tensor_t out,
                            iris_gpu_tensor_t x, const uint16_t *weight,
 ...
 * ======================================================================== */
static int qwen3_forward_vulkan(qwen3_model_t *model, int seq_len,
                                const int *attention_mask) {
    int hidden = model->hidden_size;
    int num_heads = model->num_heads;
    int num_kv_heads = model->num_kv_heads;
    ...
    int ok = hidden_t && mask_t && norm_t && q_t && k_t && v_t && attn_t &&
             gate_t && up_t && proj_t;
    for (int i = 0; i < num_saved; i++) if (!saved[i]) ok = 0;
    ...
    for (int layer_idx = 0; ok && layer_idx <= last_layer; layer_idx++) {
        ...
        if (!a->q_proj_weight_bf16 || !a->k_proj_weight_bf16 || !a->v_proj_weight_bf16 ||
            !a->o_proj_weight_bf16 || !a->q_norm_weight_bf16 || !a->k_norm_weight_bf16 ||
            !mlp->gate_proj_weight_bf16 || !mlp->up_proj_weight_bf16 || !mlp->down_proj_weight_bf16 ||
            !layer->input_layernorm_weight_bf16 || !layer->post_attention_layernorm_weight_bf16) {
            ok = 0;
            if (model->use_mmap) free_layer_weights(layer);
            break;
        }

        /* Self-attention */
        iris_vk_qwen_rms_norm(norm_t, hidden_t, layer->input_layernorm_weight_bf16,
                              seq_len, hidden, QWEN3_RMS_NORM_EPS);
        ...
        iris_vk_qwen_rope(q_t, k_t, model->rope_cos, model->rope_sin,
                          seq_len, num_heads, num_kv_heads, head_dim);
        iris_vk_qwen_attention(attn_t, q_t, k_t, v_t, mask_t,
        ...
        /* MLP (SwiGLU) */
        iris_vk_qwen_rms_norm(norm_t, hidden_t, layer->post_attention_layernorm_weight_bf16,
                              seq_len, hidden, QWEN3_RMS_NORM_EPS);
    ...
    iris_gpu_tensor_free(hidden_t); iris_gpu_tensor_free(mask_t);
    iris_gpu_tensor_free(norm_t);   iris_gpu_tensor_free(q_t);
    iris_gpu_tensor_free(k_t);      iris_gpu_tensor_free(v_t);
    iris_gpu_tensor_free(attn_t);   iris_gpu_tensor_free(gate_t);
    iris_gpu_tensor_free(up_t);     iris_gpu_tensor_free(proj_t);
    for (int i = 0; i < num_saved; i++) iris_gpu_tensor_free(saved[i]);
...
static const char *gguf_qwen3_name(const char *hf, char *out, size_t outsz) {
    if (strcmp(hf, "model.embed_tokens.weight") == 0) {
        snprintf(out, outsz, "token_embd.weight");  return out;
    }
    if (strcmp(hf, "model.norm.weight") == 0) {
        snprintf(out, outsz, "output_norm.weight"); return out;
    }
    ...
    if (sscanf(hf, "model.layers.%d.%127s", &layer, suffix) == 2) {
        const char *g = NULL;
        if      (strcmp(suffix, "input_layernorm.weight")          == 0) g = "attn_norm";
        else if (strcmp(suffix, "post_attention_layernorm.weight") == 0) g = "ffn_norm";
        else if (strcmp(suffix, "self_attn.q_proj.weight")         == 0) g = "attn_q";
        else if (strcmp(suffix, "self_attn.k_proj.weight")         == 0) g = "attn_k";
        else if (strcmp(suffix, "self_attn.v_proj.weight")         == 0) g = "attn_v";
        else if (strcmp(suffix, "self_attn.o_proj.weight")         == 0) g = "attn_output";
        else if (strcmp(suffix, "self_attn.q_norm.weight")         == 0) g = "attn_q_norm";
        else if (strcmp(suffix, "self_attn.k_norm.weight")         == 0) g = "attn_k_norm";
        else if (strcmp(suffix, "mlp.gate_proj.weight")            == 0) g = "ffn_gate";
        else if (strcmp(suffix, "mlp.up_proj.weight")              == 0) g = "ffn_up";
        else if (strcmp(suffix, "mlp.down_proj.weight")            == 0) g = "ffn_down";
        if (g) { snprintf(out, outsz, "blk.%d.%s.weight", layer, g); return out; }
    }
    return NULL;
}
...
static const char *qwen3_lookup_name(const safetensors_file_t *sf, const char *name,
 ...
 * container. */
static uint16_t *load_tensor_bf16_owned(safetensors_file_t **files, int num_files,
                                        const char *name) {
    char buf[256];
    for (int f = 0; f < num_files; f++) {
        const char *lookup = qwen3_lookup_name(files[f], name, buf, sizeof(buf));
...
qwen3_model_t *qwen3_model_load_mmap(const char *model_dir) {
    ...
    {
        char buf[256];
        const char *nm = qwen3_lookup_name(model->sf_files[0],
