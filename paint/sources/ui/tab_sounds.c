...
i32 _tab_sounds_draw_i;
i32 tab_sounds_drag_pos = -1;
...
void tab_sounds_delete_sound(slot_sound_t *sound) {
	sys_notify_on_next_frame(&tab_sounds_delete_sound_on_next_frame, sound);
...
void tab_sounds_draw_context_menu(void *_) {
	context_select_sound(_tab_sounds_draw_i);
	ui_menu_draw(&tab_sounds_draw_context_menu_draw, -1, -1);
}

void tab_sounds_draw_select_sound(void *_) {
	i32 i = _tab_sounds_draw_i;
	...
	if (ui_tab(htab, tr("Sounds"), false, -1, false) && g_ui->_window_h > ui_statusbar_default_h * UI_SCALE()) {
		...
		for (i32 row = 0; row < math_floor(math_ceil(g_project->_->sounds->length / (float)num)); ++row) {
			...
			for (i32 j = 0; j < num; ++j) {
				...
				if (base_drag_sound != NULL && tab_sounds_drag_pos == i) {
					ui_fill(-1, -2, 2, imgw_val + 4, g_theme->HIGHLIGHT_COL);
				}

				ui_state_t state = UI_STATE_IDLE;
				// if (g_project->_->sounds->buffer[i]->preview_ready) {
				...
				if (state == UI_STATE_STARTED) {
					if (g_context->sound != g_project->_->sounds->buffer[i]) {
						_tab_sounds_draw_i = i;
						sys_notify_on_next_frame(&tab_sounds_draw_select_sound, NULL);
					}
				...
				if (g_ui->is_hovered && g_ui->input_released_r) {
					_tab_sounds_draw_i = i;
					sys_notify_on_next_frame(&tab_sounds_draw_context_menu, NULL);
				}
				if (g_ui->is_hovered) {
					// if (img == NULL) {
					// _tab_sounds_draw_i = i;
					// sys_notify_on_next_frame(&tab_sounds_draw_make_font_preview, NULL);
					// }
					// else {
				...
				if (g_config->show_asset_names) {
					g_ui->_x = uix;
					g_ui->_y += slotw * 0.9;
					ui_text(g_project->_->sounds->buffer[i]->name, UI_ALIGN_CENTER, 0x00000000);
					if (g_ui->is_hovered) {
						ui_tooltip(g_project->_->sounds->buffer[i]->name);
					}
					g_ui->_y -= slotw * 0.9;
		...
		if (base_drag_sound != NULL && tab_sounds_drag_pos == g_project->_->sounds->length) {
			g_ui->_x = uix;
			g_ui->_y = uiy;
		...
		if (in_focus && g_ui->is_delete_down && g_project->_->sounds->length > 1 && !string_equals(g_context->sound->file, "")) {
			g_ui->is_delete_down = false;
			tab_sounds_delete_sound(g_context->sound);
		}
		if (in_focus) {
			i32 i = array_index_of(g_project->_->sounds, g_context->sound);
			if (g_ui->is_key_pressed && g_ui->key_code == KEY_CODE_UP) {
				if (i > 0) {
					_tab_sounds_draw_i = i - 1;
					sys_notify_on_next_frame(&tab_sounds_draw_select_sound, NULL);
				}
			}
...
void tab_sounds_accept_sound_drop(slot_sound_t *sound) {
	if (tab_sounds_drag_pos == -1) {
		return;
	}

	i32 sound_pos = array_index_of(g_project->_->sounds, sound);
	if (sound_pos != -1 && math_abs(sound_pos - tab_sounds_drag_pos) > 0) {
		array_remove(g_project->_->sounds, sound);
		i32 new_pos = tab_sounds_drag_pos - sound_pos > 0 ? tab_sounds_drag_pos - 1 : tab_sounds_drag_pos;
		array_insert(g_project->_->sounds, new_pos, sound);
	}
}
